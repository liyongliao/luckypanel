...
import (
	...
	"github.com/0xJacky/Nginx-UI/api/firewall"
	"github.com/0xJacky/Nginx-UI/api/ddns"
	"github.com/0xJacky/Nginx-UI/api/forward"
	"github.com/0xJacky/Nginx-UI/api/docker"
	"github.com/0xJacky/Nginx-UI/api/logcenter"
...
func InitRouter() {
	...
	root := r.Group("/api", middleware.IPWhiteList())
	{
		...
		g := root.Group("/", middleware.AuthRequired(), middleware.Proxy())
		{
			...
			firewall.InitRouter(g)
			ddns.InitRouter(g)
			forward.InitRouter(g)
			docker.InitRouter(g)
			logcenter.InitRouter(g)
